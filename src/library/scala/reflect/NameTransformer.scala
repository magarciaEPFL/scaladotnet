...
object NameTransformer {
  ...
  private def enterOp(op: Char, code: String) = {
    op2code(op) = code
    val c = (code.Chars(1) - 'a') * 26 + code.Chars(2) - 'a'
    ...
    while (i < len) {
      ...
      if (c < nops && (op2code(c) ne null)) {
        if (buf eq null) {
          ...
          buf.append(_root_.java.lang.String.instancehelper_substring(name, 0, i))
        }
      ...
      /* Handle glyphs that are not valid Java/JVM identifiers */
        ...
        val tmp : String = { val h = java.lang.Integer.toHexString(c.toInt); "$u" + "000".take(4 - h.size) + h }  /* MANUAL EDIT */
        buf.append(tmp)
      }
      else if (buf ne null) {
        buf.append(c)
  ...
  def decode(name0: String): String = {
    ...
    val name = if (name0.EndsWith("<init>")) _root_.java.lang.String.instancehelper_substring(name0, 0, name0.Length - ("<init>").Length) + "this"
               else name0;
    var buf: StringBuilder = null
    val len = name.Length
    var i = 0
    while (i < len) {
      ...
      val c = name Chars i
      if (c == '$' && i + 2 < len) {
        val ch1 = name.Chars(i+1)
        if ('a' <= ch1 && ch1 <= 'z') {
          val ch2 = name.Chars(i+2)
          if ('a' <= ch2 && ch2 <= 'z') {
            ops = code2op((ch1 - 'a') * 26 + ch2 - 'a')
            while ((ops ne null) && !_root_.java.lang.String.instancehelper_startsWith(name, ops.code, i)) ops = ops.next
            if (ops ne null) {
              if (buf eq null) {
                buf = new StringBuilder()
                buf.append(_root_.java.lang.String.instancehelper_substring(name, 0, i))
            ...
            val hex = _root_.java.lang.String.instancehelper_substring(name, i+2, i+6)
            try {
