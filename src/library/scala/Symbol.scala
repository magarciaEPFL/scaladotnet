...
final class Symbol private (val name: String) extends Serializable {
  ...
  override def GetHashCode = name.GetHashCode
...
private[scala] abstract class UniquenessCache[K, V >: Null]
{
  ...
  def apply(name: K): V = {
    def cached(): V = {
      rlock.lock
      try {
        val reference = (map get name).asInstanceOf[WeakReference/*[V]*/] /* MANUALLY */
        if (reference == null) null
        else (reference.get).asInstanceOf[V]  // will be null if we were gc-ed
      }
