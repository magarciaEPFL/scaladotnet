...
trait FlatHashTable[A] extends FlatHashTable.HashUtils[A] {
  ...
  protected def randomSeed = seedGenerator.get.asInstanceOf[util.Random].nextInt()
  ...
  protected def nnSizeMapAdd(h: Int) = if (sizemap ne null) {
    val p = h >> ((sizeMapBucketBitSize) & 0x1f)
    sizemap(p) += 1
  }

  protected def nnSizeMapRemove(h: Int) = if (sizemap ne null) {
    sizemap(h >> ((sizeMapBucketBitSize) & 0x1f)) -= 1
  }
  ...
  protected def calcSizeMapSize(tableLength: Int) = (tableLength >> ((sizeMapBucketBitSize) & 0x1f)) + 1
  ...
  protected final def index(hcode: Int) = {
    ...
    (improved >>> (((32 - java.lang.Integer.bitCount(ones)) & 0x1f))) & ones
...
private[collection] object FlatHashTable {
   ...
   *  See SI-5293.
   */
  final def seedGenerator = new java.lang.ThreadLocal/*[util.Random]*/ {
  ...
  trait HashUtils[A] {
    ...
    // so that:
    protected final def sizeMapBucketSize = 1 << ((sizeMapBucketBitSize) & 0x1f)

    protected def elemHashCode(elem: A) =
      if (elem == null) throw new java.lang.IllegalArgumentException("Flat hash tables cannot contain null elements.")
      else elem.GetHashCode
    ...
    protected final def improve(hcode: Int, seed: Int) = {
      ...
      val rotated = (improved >>> ((rotation) & 0x1f)) | (improved << (((32 - rotation) & 0x1f)))
      rotated
    }
  }
