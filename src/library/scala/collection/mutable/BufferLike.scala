...
{ self : This =>
  ...
  def <<(cmd: Message[A]): Unit = cmd match {
    ...
    case _                      => throw new java.lang.UnsupportedOperationException("message " + cmd + " not understood")
  }
  ...
  def ++(xs: GenTraversableOnce[A]): This = MemberwiseClone() ++= xs.seq
  ...
  override def -(elem: A): This = MemberwiseClone() -= elem
   ...
   */
  @migration("`-` creates a new buffer. Use `-=` to remove an element from this buffer and return that buffer itself.", "2.8.0")
  override def -(elem1: A, elem2: A, elems: A*): This = MemberwiseClone() -= elem1 -= elem2 --= elems
