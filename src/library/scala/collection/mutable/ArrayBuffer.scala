...
class ArrayBuffer[A](override protected val initialSize: Int)
  ...
  def insertAll(n: Int, seq: Traversable[A]) {
    if (n < 0 || n > size0) throw new java.lang.IndexOutOfBoundsException(n.ToString)
    val xs = seq.toList
    val len = xs.length
  ...
  override def remove(n: Int, count: Int) {
    ...
    if (n < 0 || n > size0 - count) throw new java.lang.IndexOutOfBoundsException(n.ToString)
   ...
   */
  override def MemberwiseClone(): ArrayBuffer[A] = new ArrayBuffer[A] ++= this
