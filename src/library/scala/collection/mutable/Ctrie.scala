...
private[mutable] final class INode[K, V](bn: MainNode/*[K, V]*/, g: Gen) extends INodeBase(g)/*[K, V]*/ {
  import INodeBase._
  ...
  @inline private def inode(cn: MainNode/*[K, V]*/) = {
    val nin = new INode[K, V](gen.asInstanceOf[Gen])
    nin.WRITE(cn)
    nin
  }
  ...
  @tailrec final def rec_insert(k: K, v: V, hc: Int, lev: Int, parent: INode[K, V], startgen: Gen, ct: Ctrie[K, V]): Boolean = {
    ...
    m match {
      case cn: CNode[K, V] => // 1) a multiway node
        val idx = (hc >>> ((lev) & 0x1f)) & 0x1f
        val flag = 1 << ((idx) & 0x1f)
        val bmp = cn.bitmap
        val mask = flag - 1
        val pos = java.lang.Integer.bitCount(bmp & mask)
        if ((bmp & flag) != 0) {
          // 1a) insert below
          cn.array(pos) match {
            case in: INode[K, V] =>
    ...
    m match {
      case cn: CNode[K, V] => // 1) a multinode
        ...
        if ((bmp & flag) == 0) null // 1a) bitmap shows no binding
          ...
          val pos = if (bmp == 0xffffffff) idx else java.lang.Integer.bitCount(bmp & (flag - 1))
    ...
    m match {
      case cn: CNode[K, V] =>
        ...
        if ((bmp & flag) == 0) None
          ...
          val pos = java.lang.Integer.bitCount(bmp & (flag - 1))
          val sub = cn.array(pos)
          val res = sub match {
            case in: INode[K, V] => 
  ...
  override def string(lev: Int) = "%sINode -> %s".format("  " * lev, mainnode match {
    case null => "<null>"
...
private[mutable] final class FailedNode[K, V](p: MainNode/*[K, V]*/) extends MainNode/*[K, V]*/ {
  ...
  def string(lev: Int) = throw new java.lang.UnsupportedOperationException
...
private[mutable] object CNode {
  ...
  def dual[K, V](x: SNode[K, V], xhc: Int, y: SNode[K, V], yhc: Int, lev: Int, gen: Gen): MainNode/*[K, V]*/ = if (lev < 35) {
    val xidx = (xhc >>> ((lev) & 0x1f)) & 0x1f
    val yidx = (yhc >>> ((lev) & 0x1f)) & 0x1f
    val bmp = (1 << ((xidx) & 0x1f)) | (1 << ((yidx) & 0x1f))
    if (xidx == yidx) {
