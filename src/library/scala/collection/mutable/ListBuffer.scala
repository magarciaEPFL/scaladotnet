...
final class ListBuffer[A]
      extends AbstractBuffer[A]
         ...
         with SeqForwarder[A]
         with java.io.Serializable.__Interface
{
  ...
  override def apply(n: Int): A =
    if (n < 0 || n >= len) throw new java.lang.IndexOutOfBoundsException(n.ToString)
    else super.apply(n)
  ...
  def update(n: Int, x: A) {
    try {
      ...
      case ex: java.lang.Exception => throw new java.lang.IndexOutOfBoundsException(n.ToString)
    }
  }
...
// Overrides of methods in Buffer
   ...
   */
  override def MemberwiseClone(): ListBuffer[A] = (new ListBuffer[A]) ++= this
