...
class BitSet(protected var elems: Array[Long]) extends AbstractSet[Int]
   ...
   */
  def this(initSize: Int) = this(new Array[Long]((initSize + 63) >> ((6) & 0x1f) max 1))
  ...
  override def add(elem: Int): Boolean = {
    ...
    if (contains(elem)) false
      ...
      val idx = elem >> ((LogWL) & 0x1f)
      updateWord(idx, word(idx) | (1L << ((elem) & 0x3f)))
      true
    }
  }
  ...
  override def MemberwiseClone(): BitSet = {
