...
{ self =>
  ...
  def + [B1 >: B] (kv: (A, B1)): Map[A, B1] = MemberwiseClone().asInstanceOf[Map[A, B1]] += kv
  ...
  override def + [B1 >: B] (elem1: (A, B1), elem2: (A, B1), elems: (A, B1) *): Map[A, B1] =
    MemberwiseClone().asInstanceOf[Map[A, B1]] += elem1 += elem2 ++= elems
  ...
  override def ++[B1 >: B](xs: GenTraversableOnce[(A, B1)]): Map[A, B1] =
    MemberwiseClone().asInstanceOf[Map[A, B1]] ++= xs.seq
  ...
  override def -(key: A): This = MemberwiseClone() -= key
  ...
  override def MemberwiseClone(): This = empty ++= repr
  ...
  override def -(elem1: A, elem2: A, elems: A*): This =
    MemberwiseClone() -= elem1 -= elem2 --= elems
   ...
   */
  @migration("`--` creates a new map. Use `--=` to remove an element from this map and return that map itself.", "2.8.0")
  override def --(xs: GenTraversableOnce[A]): This = MemberwiseClone() --= xs.seq
