...
trait SynchronizedMap[A, B] extends Map[A, B] {
  ...
  override def getOrElseUpdate(key: A, default: => B): B = (synchronized { super.getOrElseUpdate(key, default) }).asInstanceOf[B]
  ...
  override def MemberwiseClone(): Self = synchronized { super.MemberwiseClone() }
  override def foreach[U](f: ((A, B)) => U) = synchronized { super.foreach(f) }
  override def apply(key: A): B = (synchronized { super.apply(key) }).asInstanceOf[B]
