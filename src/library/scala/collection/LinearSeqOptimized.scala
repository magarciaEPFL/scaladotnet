...
trait LinearSeqOptimized[+A, +Repr <: LinearSeqOptimized[A, Repr]] extends LinearSeqLike[A, Repr] { self: Repr =>
  ...
  def apply(n: Int): A = {
    val rest = drop(n)
    if (n < 0 || rest.isEmpty) throw new java.lang.IndexOutOfBoundsException("" + n)
    rest.head
  }
  ...
  def reduceLeft[B >: A](f: (B, A) => B): B =
    if (isEmpty) throw new java.lang.UnsupportedOperationException("empty.reduceLeft")
    else tail.foldLeft[B](head)(f)

  override /*IterableLike*/
  def reduceRight[B >: A](op: (A, B) => B): B =
    if (isEmpty) throw new java.lang.UnsupportedOperationException("Nil.reduceRight")
