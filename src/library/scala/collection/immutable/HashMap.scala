  ...
  class HashMap1[A,+B](private[HashMap] var key: A, private[HashMap] var hash: Int, private[collection] var value: (B @uV), private[collection] var kv: (A,B @uV)) extends HashMap[A,B] {
    ...
    override def updated0[B1 >: B](key: A, hash: Int, level: Int, value: B1, kv: (A, B1), merger: Merger[B1]): HashMap[A, B1] =
      if (hash == this.hash && key == this.key ) {
        ...
        var thatindex = (hash >>> ((level) & 0x1f)) & 0x1f
        var thisindex = (this.hash >>> ((level) & 0x1f)) & 0x1f
        if (hash != this.hash) {
          ...
          while (thisindex == thatindex) {
            val newlevel = new HashTrieMap[A, B1](1 << ((thisindex) & 0x1f), new Array[HashMap[A, B1]](1), 2)
            if (prev ne null) prev.elems(0) = newlevel else top = newlevel
            prev = newlevel
            lvl += 5
            thatindex = (hash >>> ((lvl) & 0x1f)) & 0x1f
            thisindex = (this.hash >>> ((lvl) & 0x1f)) & 0x1f
          }
          ...
          val bottom = new HashTrieMap[A,B1]((1 << ((thisindex) & 0x1f)) | (1 << ((thatindex) & 0x1f)), bottelems, 2)
          if (prev ne null) {
            prev.elems(0) = bottom
            top
          } else bottom
...
*/
    ...
    override def get0(key: A, hash: Int, level: Int): Option[B] = {
      val index = (hash >>> ((level) & 0x1f)) & 0x1f
      val mask = (1 << ((index) & 0x1f))
      if (bitmap == - 1) {
        elems(index & 0x1f).get0(key, hash, level + 5)
      } else if ((bitmap & mask) != 0) {
        val offset = java.lang.Integer.bitCount(bitmap & (mask-1))
...
*/
    ...
    override def split: Seq[HashMap[A, B]] = if (size == 1) Seq(this) else {
      val nodesize = java.lang.Integer.bitCount(bitmap)
      if (nodesize > 1) {
        val splitpoint = nodesize / 2
        val bitsplitpoint = posOf(nodesize / 2, bitmap)
        val bm1 = bitmap & (-1 << ((bitsplitpoint) & 0x1f))
        val bm2 = bitmap & (-1 >>> (((32 - bitsplitpoint) & 0x1f)))
    ...
    protected override def merge0[B1 >: B](that: HashMap[A, B1], level: Int, merger: Merger[B1]): HashMap[A, B1] = that match {
      ...
      case hm: HashTrieMap[_, _] =>
        ...
        val subcount = java.lang.Integer.bitCount(thisbm | thatbm)
