...
extends AbstractSeq[T] with IndexedSeq[T] with Serializable {
  ...
  def apply(idx: Int): T = {
    if (idx < 0 || idx >= length) throw new java.lang.IndexOutOfBoundsException(idx.ToString)
  ...
  override lazy val GetHashCode = super.GetHashCode
...
object NumericRange {
  ...
  def count[T](start: T, end: T, step: T, isInclusive: Boolean)(implicit num: Integral[T]): Int = {
    ...
    if (step == zero) throw new java.lang.IllegalArgumentException("step cannot be 0.")
      ...
      if (longCount > scala.Int.MaxValue || longCount < 0L || isOverflow) {
        ...
        throw new java.lang.IllegalArgumentException(descr + ": seqs cannot contain more than Int.MaxValue elements.")
      }
      longCount.toInt
    }
  }
