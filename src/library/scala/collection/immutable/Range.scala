   ...
   with Serializable
{
  ...
  val numRangeElements: Int = {
    if (step == 0) throw new java.lang.IllegalArgumentException("step cannot be 0.")
    else if (isEmpty) 0
    else {
      val len = longLength
  ...
  private def fail() = throw new java.lang.IllegalArgumentException(description + ": seqs cannot contain more than Int.MaxValue elements.")
  ...
  @inline final def apply(idx: Int): Int = {
    validateMaxLength()
    if (idx < 0 || idx >= numRangeElements) throw new java.lang.IndexOutOfBoundsException(idx.ToString)
    else start + (step * idx)
  }
...
object Range {
  ...
  def count(start: Int, end: Int, step: Int, isInclusive: Boolean): Int = {
    if (step == 0)
      throw new java.lang.IllegalArgumentException("step cannot be 0.")
