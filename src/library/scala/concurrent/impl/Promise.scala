...
object Promise {
  ...
  case class Failure[T](value: Option[util.Failure[T]] = None) extends FState[T] {
    def exception: System.Exception = value.get.exception
  }
  ...
  self =>
    ...
    protected final def tryAwait(atMost: Duration): Boolean = {
      ...
      def awaitUnsafe(waitTimeNanos: Long): Boolean = {
        if (value.isEmpty && waitTimeNanos > 0) {
          ...
          try {
            synchronized {
              while (value.isEmpty) _root_.java.lang.Object.instancehelper_wait( this , ms, ns ) 
            }
          } catch {
            case e: java.lang.InterruptedException =>
          }

          awaitUnsafe(waitTimeNanos - (java.lang.System.nanoTime() - start))
        } else
          value.isDefined
      }
    ...
    @inline
    protected final def getState: FState[T] = updater.get(this).asInstanceOf[FState[T]]
    ...
    def tryComplete(value: Try[T]): Boolean = {
      val callbacks: List[Try[T] => Any] = {
        try {
          ...
          synchronized { _root_.java.lang.Object.instancehelper_notifyAll(this) } // notify any blockers from `tryAwait`
        }
      }
