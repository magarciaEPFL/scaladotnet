...
object ops
{
   ...
   */
  private def tryCatch[A](body: => A): Either[System.Exception, A] =
    allCatch[A] either body

  private def getOrThrow[T <: System.Exception, A](x: Either[T, A]): A =
  ...
  def par[A, B](xp: => A, yp: => B)(implicit runner: TaskRunner = defaultRunner): (A, B) = {
    val y = new SyncVar[Either[System.Exception, B]]
