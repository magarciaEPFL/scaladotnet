...
trait Promise[T] {
   ...
   *  $promiseCompletion
   */
  def failure(t: System.Exception): this.type = if (tryFailure(t)) this else throwCompleted
   ...
   *  @return    If the promise has already been completed returns `false`, or `true` otherwise.
   */
  def tryFailure(t: System.Exception): Boolean = tryComplete(Failure(t))
  
  /** Wraps a `Throwable` in an `ExecutionException` if necessary. TODO replace with `resolver` from scala.concurrent
   *  
   *  $allowedThrowables
   */
  protected def wrap(t: System.Exception): System.Exception = t match {
    case t: System.Exception if isFutureThrowable(t) => t
    case _ => new ExecutionException(t)
  }
  
  private def throwCompleted = throw new java.lang.IllegalStateException("Promise already completed.")
...
object Promise {
  ...
  def broken[T](t: System.Exception)(implicit execctx: ExecutionContext): Promise[T] = 
