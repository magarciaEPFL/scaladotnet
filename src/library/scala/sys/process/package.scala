...
package scala.sys {
  ...
  package object process extends ProcessImplicits {
    def javaVmArguments: List[String] = {
      ...
      // TODO we don't want to add a 1MB dependency (IKVM.OpenJDK.Management.dll) due to next call
  ...
  package process {
    ...
    private[process] object processInternal {
      ...
      def onInterrupt[T](handler: => T): System.Exception =?> T = {
        case _: java.lang.InterruptedException => handler
      }

      def ioFailure[T](handler: IOException => T): System.Exception =?> T = {
