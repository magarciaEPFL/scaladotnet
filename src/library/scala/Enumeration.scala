...
abstract class Enumeration (initial: Int) extends Serializable {
  ...
  protected def readResolve(): AnyRef = _root_.java.lang.Object.instancehelper_getClass(thisenum).getField(MODULE_INSTANCE_NAME).get(null)

  /** The name of this enumeration.
   */
  override def ToString = (
    _root_.java.lang.String.instancehelper_split(_root_.java.lang.Object.instancehelper_getClass(this).getName stripSuffix MODULE_SUFFIX_STRING split '.' last, Pattern.quote(NAME_JOIN_STRING)) last
  )
  ...
  private def populateNameMap() {
    val fields = _root_.java.lang.Object.instancehelper_getClass(this).getDeclaredFields
    def isValDef(m: JMethod) = fields exists (fd => fd.getName == m.getName && fd.getType == m.getReturnType)

    // The list of possible Value methods: 0-args which return a conforming type
    val methods = _root_.java.lang.Object.instancehelper_getClass(this).getMethods filter (m => m.getParameterTypes.isEmpty &&
                                                   classOf[Value].isAssignableFrom(m.getReturnType) &&
                                                   class2type(m.getDeclaringClass) != classOf[Enumeration] &&  /* MANUALLY */
                                                   isValDef(m))
    methods foreach { m =>
      val name = m.getName
  ...
  abstract class Value extends Ordered[Value] with Serializable {
    ...
    override def GetHashCode: Int = id.##
  ...
  object ValueOrdering extends Ordering[Value] {
    def compareNonErased(x: Value, y: Value): Int = x.id - y.id
    override def compare(x: Any, y: Any) = { this.compareNonErased(x.asInstanceOf[Value], y.asInstanceOf[Value]) }
  }
