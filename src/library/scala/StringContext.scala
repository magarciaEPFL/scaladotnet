...
case class StringContext(parts: String*) {
  ...
  def checkLengths(args: Any*): Unit =
    if (parts.length != args.length + 1)
      throw new java.lang.IllegalArgumentException("wrong number of arguments for interpolated string")
  ...
  def f(args: Any*) = {
    ...
    def copyString(first: Boolean): Unit = {
      ...
      while (idx < len) {
        if (str(idx) == '%') {
          bldr append (_root_.java.lang.String.instancehelper_substring(str, start, idx)) append "%s"
          args1 += "%"
          start = idx + 1
        }
        idx += 1
      }
      if (!strIsEmpty) bldr append (_root_.java.lang.String.instancehelper_substring(str, start, idx))
    }
...
object StringContext {
  ...
  def treatEscapes(str: String): String = {
    ...
    if (start == 0) str
    else (bldr append _root_.java.lang.String.instancehelper_substring(str, start, idx)).ToString
  }
}
