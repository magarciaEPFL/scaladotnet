...
case class IntDef(value:String) extends EntityDef {
  private def validateValue() {
    ...
    while (ix != -1) {
      ...
      if(iz == -1 && iz == ix + 1)
        throw new java.lang.IllegalArgumentException("no % allowed in entity value, except for parameter-entity-references")
      else {
        val n = _root_.java.lang.String.instancehelper_substring(tmp, ix, iz)

        if (!Utility.isName(n))
          throw new java.lang.IllegalArgumentException("internal entity def: \""+n+"\" must be an XML Name")

        tmp = _root_.java.lang.String.instancehelper_substring(tmp, iz+1, tmp.Length)
        ix  = tmp IndexOf '%'
      }
    }
  }
...
case class PEReference(ent:String) extends MarkupDecl {
  if( !Utility.isName( ent ))
    throw new java.lang.IllegalArgumentException("ent must be an XML Name");
