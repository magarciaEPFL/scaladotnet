...
extends collection.AbstractIterator[XMLEvent]
  ...
  private[this] val parserThread = new java.lang.Thread(parser, "XMLEventReader")
  parserThread.start
...
trait ProducerConsumerIterator[T >: Null] extends Iterator[T] {
  ...
  def interruptibly[T](body: => T): Option[T] = try Some(body) catch {
    case _: java.lang.InterruptedException    => java.lang.Thread.currentThread.interrupt(); None
  ...
  private def fillBuffer() = {
    buffer = interruptibly((queue.take).asInstanceOf[T]) getOrElse EndOfStream
    isElement(buffer)
  }
  ...
  def available() = isElement(buffer) || isElement((queue.peek).asInstanceOf[T])
