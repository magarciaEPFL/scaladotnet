...
class PrettyPrinter(width: Int, step: Int) {
  ...
  protected def cut(s: String, ind: Int): List[Item] = {
    ...
    var res: List[Item] = Nil
      ...
      def loopBody() = {
        try {
          val b = Box(ind, _root_.java.lang.String.instancehelper_substring(s, 0, last.head))
          cur = ind
          res = b :: Break :: cut(_root_.java.lang.String.instancehelper_substring(s, last.head, s.Length), ind)
          // backtrack
          last = last.tail
        } catch {
      ...
      case _ =>
        ...
        if (childrenAreLeaves(node) && fits(test)) {
          ...
          if (stg.Length < width - cur) { // start tag fits
            ...
            makeBox(ind, _root_.java.lang.String.instancehelper_substring(stg, 0, len2))
            ...
            makeBox(ind, _root_.java.lang.String.instancehelper_substring(stg, len2, stg.Length))
            makeBreak()
            traverse(node.child.iterator, node.scope, ind + step)
            makeBox(cur, etg)
            makeBreak()
