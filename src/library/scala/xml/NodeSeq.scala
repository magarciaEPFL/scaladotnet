...
abstract class NodeSeq extends AbstractSeq[Node] with immutable.Seq[Node] with SeqLike[Node, NodeSeq] with Equality {
  ...
  def \(that: String): NodeSeq = {
    def fail = throw new java.lang.IllegalArgumentException(that)
    def atResult = {
      lazy val y = this(0)
      val attr =
        if (that.Length == 1) fail
          ...
          val i = that IndexOf '}'
          if (i == -1) fail
          val (uri, key) = (_root_.java.lang.String.instancehelper_substring(that, 2,i), _root_.java.lang.String.instancehelper_substring(that, i+1, that.Length))
          if (uri == "" || key == "") fail
