...
// ------------------ Reporting -------------------------------------
  ...
  def logError(msg: String, t: System.Exception): Unit = ()
...
// ------------ File interface -----------------------------------------
  ...
  private val reader: SourceReader = {
    ...
    def loadReader(name: String): Option[SourceReader] = {
      def ccon = java.lang.Class.forName(name).getConstructor(scala.Array( classOf[CharsetDecoder], classOf[Reporter] ) )

      try Some(ccon.newInstance(scala.Array( charset.newDecoder(), reporter ) ).asInstanceOf[SourceReader])
      catch { case x =>
...
// ------------ Phases -------------------------------------------}
    ...
    def run() {
      ...
      currentRun.units foreach { unit =>
        if (opt.timings) {
          ...
          try applyPhase(unit)
          finally unitTimings(unit) += (java.lang.System.nanoTime - start)
        }
  ...
  catch { case x: java.lang.Exception => errorMessage }
  ...
  class Run {
    ...
    def resetProjectClasses(root: Symbol): Unit = try {
      ...
      case ex: System.Exception =>
        ...
        java.lang.Throwable.instancehelper_printStackTrace(ex);
        throw ex
    }
    ...
    lazy val profiler = java.lang.Class.forName(opt.profileClass).newInstance().asInstanceOf[Profiling]
    ...
    def compileUnits(units: List[CompilationUnit], fromPhase: Phase) {
      try compileUnitsInternal(units, fromPhase)
        ...
        globalError(supplementErrorMessage("uncaught exception during compilation: " + _root_.java.lang.Object.instancehelper_getClass(ex).getName))
        throw ex
      }
    }
    ...
    def compileFiles(files: List[AbstractFile]) {
      ...
      catch { case ex: IOException => globalError(ex.Message) }
    }
...
//        file = new File(file.getParentFile(), file.getName() + "1")
      try {
        ...
        case ex: IOException =>
          if (opt.debug) java.lang.Throwable.instancehelper_printStackTrace(ex)
...
object Global {
  ...
  def fromSettings(settings: Settings, reporter: Reporter): Global = {
    ...
    val parentLoader = settings.explicitParentLoader getOrElse _root_.java.lang.Object.instancehelper_getClass(this).getClassLoader
    ...
    val clazz        = java.lang.Class.forName(name, true, loader)
    val cons         = clazz.getConstructor(scala.Array( classOf[Settings], classOf[Reporter] ) )

    cons.newInstance(scala.Array( settings, reporter ) ).asInstanceOf[Global]
  }
