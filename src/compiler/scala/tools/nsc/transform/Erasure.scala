...
// -------- erasure on types --------------------------------------------------------
  ...
  def getClassReturnType(tpe: Type): Type = {
    if (phase.erasedTypes || forMSIL) ClassClass.tpe else {
            ...
            dotCleanup(
              (
                if (needsJavaSig(preRebound)) {
                  ...
                  if (s.Chars(0) == 'L') _root_.java.lang.String.instancehelper_substring(s, 0, s.Length - 1) + "." + sym.javaSimpleName
  ...
  class UnknownSig extends java.lang.Exception
    ...
    override protected def typed1(tree: Tree, mode: Int, pt: Type): Tree = {
      val tree1 = try {
        ...
        case er: TypeError =>
          ...
          java.lang.Throwable.instancehelper_printStackTrace(er)
          abort()
