...
abstract class LiftCode extends Transform with TypingTransformers {
  ...
  private def mirrorFactoryName(value: Any): Option[String] = value match {
    ...
    case _ =>
      Some(_root_.java.lang.String.instancehelper_split(value.GetType.getName, """[$.]""") last) filter MirrorMemberNames
  }
  ...
  class Codifier(unit: CompilationUnit) extends TypingTransformer(unit) {
    ...
    def printCopypaste(tree: Tree) {
      ...
      for (line <- (_root_.java.lang.String.instancehelper_split(tree.ToString, Properties.lineSeparator) drop 2 dropRight 1)) {
        var s = line.Trim
        ...
        s = """Modifiers\((\d+)L, newTypeName\("(.*?)"\), List\((.*?)\)\)""".r.replaceAllIn(s, m => {
          val buf = new _root_.java.lang.StringBuffer()
          ...
          var s = buf.ToString
          if (s.EndsWith(", List()")) s = _root_.java.lang.String.instancehelper_substring(s, 0, s.Length - ", List()".Length)
          if (s.EndsWith(", newTypeName(\"\")")) s = _root_.java.lang.String.instancehelper_substring(s, 0, s.Length - ", newTypeName(\"\")".Length)
          if (s.EndsWith("Set()")) s = _root_.java.lang.String.instancehelper_substring(s, 0, s.Length - "Set()".Length)
          "Modifiers(" + s + ")"
        })
...
*/
    ...
    private def path(fullname: String, mkName: String => Name): Tree = {
      val parts = _root_.java.lang.String.instancehelper_split(fullname, "\\.")
    ...
    private def cannotReify(value: Any): Nothing =
      abort("don't know how to reify " + value + " of class " + value.GetType) /* MANUALLY */
  }
}
