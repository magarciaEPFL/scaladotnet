...
abstract class CleanUp extends Transform with ast.TreeDSL {
  ...
  class CleanUpTransformer(unit: CompilationUnit) extends Transformer {
    ...
    private def savingStatics[T](body: => T): T = {
      val savedNewStaticMembers : mutable.Buffer[Tree] = newStaticMembers.MemberwiseClone()
      val savedNewStaticInits   : mutable.Buffer[Tree] = newStaticInits.MemberwiseClone()
      val savedSymbolsStoredAsStatic : mutable.Map[String, Symbol] = symbolsStoredAsStatic.MemberwiseClone()        
      val result = body
