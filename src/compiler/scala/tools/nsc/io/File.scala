...
object File {
  ...
  // }
  // catch {
  //   case _: IllegalArgumentException | _: java.lang.IllegalStateException | _: java.lang.IOException | _: java.lang.SecurityException => ()
  //   case _ => () // MANUALLY 
  // }
}
import File._
...
class File(jfile: JFile)(implicit constructorCodec: Codec) extends Path(jfile) with Streamable.Chars {
  ...
  def setExecutable(executable: Boolean, ownerOnly: Boolean = true): Boolean = {
    ...
    val method =
      try classOf[JFile].getMethod("setExecutable", scala.Array( classOf[Boolean], classOf[Boolean] ) )
      catch { case _: java.lang.NoSuchMethodException => return false }

    try method.invoke(jfile, scala.Array( executable: JBoolean, ownerOnly: JBoolean ) ).asInstanceOf[JBoolean].booleanValue
    catch { case _: java.lang.Exception => false }
  }
}
