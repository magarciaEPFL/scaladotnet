...
class Jar(file: File) extends Iterable[JarEntry] {
  ...
  def classPathElements: List[String] = classPathString match {
    case Some(s)  => _root_.java.lang.String.instancehelper_split(s, "\\s+") toList
    case _        => Nil
  }
  ...
  def fileishIterator: Iterator[Fileish] = jarFile.entries.asScala map (x => Fileish(x.asInstanceOf[JarEntry], () => getEntryStream(x.asInstanceOf[JarEntry]))) /* MANUAL EDIT */
...
object Jar {
  ...
  class WManifest(manifest: JManifest) {
    ...
    def attrs = manifest.getMainAttributes().asInstanceOf[AttributeMap].asScala.asInstanceOf[collection.mutable.Map[Attributes.Name, String]] withDefaultValue null
