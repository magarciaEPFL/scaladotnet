...
object Pickler {
    ...
    new CondPickler[T](x => p.canPickle(x) || q.canPickle(x)) {
      ...
      def pickle(wr: Writer, x: T) =
        require(tryPickle(wr, x),
                "no pickler found for "+x+" of class "+_root_.java.lang.Object.instancehelper_getClass(x.asInstanceOf[AnyRef]).getName)
  ...
  def singletonPickler[T <: AnyRef](x: T): CondPickler[T] =
    unitPickler
      .wrapped { _ => x } { x => () }
      .labelled (_root_.java.lang.Object.instancehelper_getClass(x).getName)
  ...
  def javaInstancePickler[T <: AnyRef]: Pickler[T] =
    (stringPickler labelled "$new")
      .wrapped { name => java.lang.Class.forName(name).newInstance().asInstanceOf[T] } { _root_.java.lang.Object.instancehelper_getClass(_).getName }
