...
object Path {
  ...
  def extension(name: String): String = {
    var i = name.Length - 1
    while (i >= 0 && name.Chars(i) != '.')
      i -= 1

    if (i < 0) ""
    else name.Substring(i + 1).ToLower
  }
...
class Path private[io] (val jfile: JFile) {
  ...
  def hasExtension(ext: String, exts: String*) = {
    val lower = extension.ToLower
    ext.ToLower == lower || exts.exists(_.ToLower == lower)
  }
  ...
  def notExists = try !jfile.exists() catch { case ex: java.lang.SecurityException => false }
  ...
  override def GetHashCode() = path.GetHashCode
}
