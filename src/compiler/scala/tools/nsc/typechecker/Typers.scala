...
/* --- end unapply  --- */
    ...
    @inline final def printTyping(s: => String) = {
      if (printTypings)
        println(context.typingIndent + _root_.java.lang.String.instancehelper_replaceAll(s, "\n", "\n" + context.typingIndent))
    }
...
//        case Ident(name) => ??
      ...
      def typedBind(name: Name, body: Tree) = {
        ...
        def typedBindType(name: TypeName) = {
          assert(body == EmptyTree, context.unit + " typedBind: " + name.debugString + " " + body + " " + _root_.java.lang.Object.instancehelper_getClass(body))
          if (vble == NoSymbol)
            vble =
...
*/
        ...
        if (!reallyExists(sym)) {
          ...
          if (settings.debug.value) {
            log(
              ...
              "\nscope-id = "+qual.tpe.termSymbol.info.decls.GetHashCode+"\nmembers = "+qual.tpe.members+
      ...
      tree match {
        ...
        case _ =>
          abort("unexpected tree: " + _root_.java.lang.Object.instancehelper_getClass(tree) + "\n" + tree)//debug
      }
    }
    ...
    def typed(tree: Tree, mode: Int, pt: Type): Tree = {
      ...
      try {
        if (Statistics.enabled) {
          ...
          typerTime = t
          pendingTreeTypes = _root_.java.lang.Object.instancehelper_getClass(tree) :: pendingTreeTypes
        }
        if (context.retyping &&
...
//      if ((mode & TYPEmode) != 0) println("type: "+tree1+" has type "+tree1.tpe)
    ...
    def expandMacro(tree: Tree): Tree = try {
      macroExpand(tree) match {
        case t: Tree => t
        case t => errorTree(tree, "macros must return a compiler-specific tree; returned class is: " + t.GetType)
      }
    } catch {
