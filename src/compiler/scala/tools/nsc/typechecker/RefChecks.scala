...
// Transformation ------------------------------------------------------------
    ...
    private def checkBounds(tree0: Tree, pre: Type, owner: Symbol, tparams: List[Symbol], argtps: List[Type]): Unit =
      try typer.infer.checkBounds(tree0, pre, owner, tparams, argtps, "")
        ...
        case ex: TypeError =>
          unit.error(tree0.pos, ex.Message)
    ...
    override def transform(tree: Tree): Tree = {
      ...
      try {
          ...
          case Ident(name) =>
            transformCaseApply(tree,
              if (name != nme.WILDCARD && name != tpnme.WILDCARD_STAR) {
                assert(sym != NoSymbol, "transformCaseApply: name = " + name.debugString + " tree = " + tree + " / " + _root_.java.lang.Object.instancehelper_getClass(tree)) //debug
        ...
        case ex: TypeError =>
          ...
          unit.error(tree.pos, ex.Message)
          tree
      } finally {
