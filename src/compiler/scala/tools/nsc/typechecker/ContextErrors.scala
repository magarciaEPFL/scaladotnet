...
trait ContextErrors {
  ...
  trait AbsTypeError extends java.lang.Throwable {
    def errPos: Position
    def errMsg: String
    def kind: ErrorKind
  }
  ...
  trait TyperContextErrors {
    ...
    object TyperErrorGen {
      ...
      def ParentTypesError(templ: Template, ex: TypeError) = {
         templ.tpe = null
         issueNormalTypeError(templ, ex.Message)
      }
    ...
    // checkNoDoubleDefs...
     ...
     def MacroExpandError(tree: Tree, t: Any) = { 
       issueNormalTypeError(tree, "macros must return a compiler-specific tree; returned class is: " + t.GetType)
       setError(tree)
     }
    }
  }
      ...
      def TypeSigError(tree: Tree, ex: TypeError) = {
        ex match {
          case CyclicReference(sym, info: TypeCompleter) =>
            issueNormalTypeError(tree, typer.cyclicReferenceMessage(sym, info.tree) getOrElse ex.Message)
          case _ =>
