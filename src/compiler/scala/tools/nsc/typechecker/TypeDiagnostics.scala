...
trait TypeDiagnostics {
  ...
  trait TyperDiagnostics {
    ...
    private def contextError(pos: Position, err: System.Exception) = context.error(pos, err)
    ...
    def reportTypeError(pos: Position, ex: TypeError) {
      ...
      ex match {
        case CyclicReference(sym, info: TypeCompleter) =>
          contextError(ex.pos, cyclicReferenceMessage(sym, info.tree) getOrElse ex.Message)
