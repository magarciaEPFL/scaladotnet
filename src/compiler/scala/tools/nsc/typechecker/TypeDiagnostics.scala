...
trait TypeDiagnostics {
  ...
  trait TyperDiagnostics {
    ...
    private def contextError(context0: Analyzer#Context, pos: Position, err: System.Exception) = context0.error(pos, err)
    ...
    def reportTypeError(context0: Context, pos: Position, ex: TypeError) {
      ...
      ex match {
        case CyclicReference(sym, info: TypeCompleter) =>
          ...
          contextError(context0, pos, cyclicReferenceMessage(sym, info.tree) getOrElse ex.Message)
