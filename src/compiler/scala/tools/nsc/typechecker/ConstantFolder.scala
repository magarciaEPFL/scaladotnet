...
abstract class ConstantFolder {
  ...
  private def foldSubrangeOp(op: Name, x: Constant, y: Constant): Constant = op match {
    ...
    case nme.LSL => Constant(x.intValue << ((y.intValue) & 0x1f))
    case nme.LSR => Constant(x.intValue >>> ((y.intValue) & 0x1f))
    case nme.ASR => Constant(x.intValue >> ((y.intValue) & 0x1f))
  ...
  private def foldLongOp(op: Name, x: Constant, y: Constant): Constant = op match {
    ...
    case nme.LSL => Constant(x.longValue << ((y.longValue) & 0x3f))
    case nme.LSR => Constant(x.longValue >>> ((y.longValue) & 0x3f))
    case nme.ASR => Constant(x.longValue >> ((y.longValue) & 0x3f))
