    ...
    private def putEntry(entry: AnyRef): Boolean = index.get(entry) match {
      ...
      case None =>
        if (ep == entries.length) {
          ...
          java.lang.System.arraycopy(entries, 0, entries1, 0, ep)
          entries = entries1
        }
        entries(ep) = entry
        index(entry) = ep
...
//          }
        ...
        case _ =>
          throw new FatalError("bad type: " + tp + "(" + _root_.java.lang.Object.instancehelper_getClass(tp) + ")")
      }
    }
    ...
    private def writeEntry(entry: AnyRef) {
      def writeBody(entry: AnyRef): Int = entry match {
        ...
        case c @ Constant(_) =>
          ...
          else if (c.tag == FloatTag) writeLong(java.lang.Float.floatToIntBits(c.floatValue))
          else if (c.tag == DoubleTag) writeLong(java.lang.Double.doubleToLongBits(c.doubleValue))
        ...
        case _ =>
          throw new FatalError("bad entry: " + entry + " " + _root_.java.lang.Object.instancehelper_getClass(entry))
      }
