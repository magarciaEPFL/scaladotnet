...
abstract class ClassfileParser {
  ...
  def parse(file: AbstractFile, root: Symbol) = try {
    ...
    def handleMissing(e: MissingRequirementError) = {
      if (settings.debug.value) java.lang.Throwable.instancehelper_printStackTrace(e)
      throw new IOException("Missing dependency '" + e.req + "', required by " + in.file)
    }

    def handleError(e: java.lang.Exception) = {
      if (settings.debug.value) java.lang.Throwable.instancehelper_printStackTrace(e)
      throw new IOException("class file '" + in.file + "' is broken\n(" + {
        if (e.Message != null) e.Message
        else _root_.java.lang.Object.instancehelper_getClass(e).ToString
      } + ")")
    }
  ...
  private def parseHeader() {
    ...
    if (magic != JAVA_MAGIC)
      throw new IOException("class file '" + in.file + "' "
                            + "has wrong magic number 0x" + java.lang.Integer.toHexString(magic)
                            + ", should be 0x" + java.lang.Integer.toHexString(JAVA_MAGIC))
...
//            println("Looking for: " + name + ": " + tpe + " inside: " + ownerTpe.typeSymbol + "\n\tand found: " + ownerTpe.members)
    ...
    private def getSubArray(bytes: Array[Byte]): Array[Byte] = {
      ...
      java.lang.System.arraycopy(bytes, 0, arr, 0, decodedLength)
      arr
    }
    ...
    def getBytes(index: Int): Array[Byte] = {
      ...
      if (value eq null) {
        ...
        java.lang.System.arraycopy(in.buf, start + 3, bytes, 0, len)
    ...
    private def errorBadIndex(index: Int) =
      throw new java.lang.RuntimeException("bad constant pool index: " + index + " at pos: " + in.bp)

    /** Throws an exception signaling a bad tag at given address. */
    private def errorBadTag(start: Int) =
      throw new java.lang.RuntimeException("bad constant pool tag " + in.buf(start) + " at byte " + start)
  }
          ...
          if (isScalaAnnot || !isScala) {
            ...
            if (isScalaAnnot)
              scalaSigAnnot match {
                ...
                case None =>
                  throw new java.lang.RuntimeException("Scala class file does not contain Scala annotation")
              }
