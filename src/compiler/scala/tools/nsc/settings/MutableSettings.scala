...
class MutableSettings(val errorFn: String => Unit)
  ...
  protected def parseParams(args: List[String]): List[String] = {
    ...
    def parseColonArg(s: String): Option[List[String]] = {
      ...
      tryToSetIfExists(p, _root_.java.lang.String.instancehelper_split(args, ",") toList, (s: Setting) => s.tryToSetColon _)
    }
    ...
    args match {
      ...
      case arg :: rest  =>
        if (!arg.StartsWith("-")) {
          ...
          else if (_root_.java.lang.String.instancehelper_contains(arg, ":")) parseColonArg(arg) match {
            case Some(_)  => rest
            case None     => args
          }
  ...
  * and `boot.class.path`.  These resources should contain the application
  * and boot classpaths in the same form as would be passed on the command line.*/
  def embeddedDefaults(loader: java.lang.ClassLoader) {
  ...
  private[nsc] var explicitParentLoader: Option[java.lang.ClassLoader] = None

  /** Retrieves the contents of resource "${id}.class.path" from `loader`
  * (wrapped in Some) or None if the resource does not exist.*/
  private def getClasspath(id: String, loader: java.lang.ClassLoader): Option[String] =
  ...
  extends Setting(name, descr) {
    ...
    override def tryToSetFromPropertyValue(s : String) {
      value = _root_.java.lang.String.instancehelper_equalsIgnoreCase(s, "true")
    }
  }
  ...
  extends Setting(name, descr) {
    ...
    override def tryToSetFromPropertyValue(s: String) = tryToSet(_root_.java.lang.String.instancehelper_split(s.Trim, " +").toList)
