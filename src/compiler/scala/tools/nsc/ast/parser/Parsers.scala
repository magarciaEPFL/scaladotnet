    ...
    def ident(skipIt: Boolean): Name =
      if (isIdent) {
        val name =
          if(forMSIL) {
            val n = in.name
            if (_root_.java.lang.String.instancehelper_contains(n.ToString, "\u0060") || n.ToString.Equals("__<ref>")) n
            else n.encode
          } else in.name.encode
        in.nextToken()
        name
    ...
    */
    ...
    def literal(isNegated: Boolean = false): Tree = {
      ...
      if (in.token == SYMBOLLIT)
        ...
        case STRINGLIT | STRINGPART => _root_.java.lang.String.instancehelper_intern(in.strVal)
        case TRUE      => true
        case FALSE     => false
        case NULL      => null
        case _         =>
