...
private[tests] trait TestSettings {
  ...
  protected val outDir = Path(Option(java.lang.System.getProperty("partest.cwd")).getOrElse("."))

  /** The base directory for this test, usually a subdirectory of "test/files/presentation/" */
  protected val baseDir = Option(java.lang.System.getProperty("partest.testname")).map(outDir / _).getOrElse(Path("."))
