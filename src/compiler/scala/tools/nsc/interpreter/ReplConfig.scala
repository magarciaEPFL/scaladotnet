...
trait ReplConfig {
  ...
  private def echo(msg: => String) =
    ...
    catch { case x: java.lang.AssertionError => Console.println("Assertion error printing debugging output: " + x) }
  ...
  private[nsc] def logAndDiscard[T](label: String, alt: => T): PartialFunction[System.Exception, T] = {
    case t =>
