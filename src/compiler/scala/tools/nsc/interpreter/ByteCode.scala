...
object ByteCode {
  ...
  private def decoderMethod(name: String, args: JClass*): Option[reflect.Method] = {
    for (decoder <- DECODER ; m <- Option(_root_.java.lang.Object.instancehelper_getClass(decoder).getMethod(name, scala.Array( args: _* ) ))) yield m
  }

  private lazy val aliasMap = {
    for (module <- DECODER ; method <- decoderMethod("typeAliases", classOf[String])) yield
      { p : String => method.invoke(module, scala.Array( p )  ).asInstanceOf[Option[Map[String, String]]] }
  }
  ...
  def scalaSigBytesForPath(path: String) =
    for {
      ...
      names <- method.invoke(module, scala.Array( path ) ).asInstanceOf[Option[Array[Byte]]]
    }
    yield names
  ...
  def caseParamNamesForPath(path: String) =
    for {
      ...
      names <- method.invoke(module, scala.Array( path ) ).asInstanceOf[Option[List[String]]]
    }
    yield names
