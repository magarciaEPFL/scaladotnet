                   ...
                   with ILoopInit
{
  ...
  protected val originalClassLoader = java.lang.Thread.currentThread.getContextClassLoader
  ...
  lazy val historyCommand = new LoopCommand("history", "show the history (optional num is commands to show)") {
    ...
    def apply(line: String): Result = {
      ...
      val count   = try xs.head.toInt catch { case _: java.lang.Exception => defaultLines }
  ...
  def searchHistory(_cmdline: String) {
    val cmdline = _cmdline.ToLower
    val offset  = history.index - history.size + 1

    for ((line, index) <- history.asStrings.zipWithIndex ; if _root_.java.lang.String.instancehelper_contains(line.ToLower, cmdline))
  ...
  private def implicitsCommand(line: String): Result = {
    ...
    val args     = _root_.java.lang.String.instancehelper_split(line, "\\s+")
    val filtered = intp.implicitSymbolsBySource filter {
      case (source, syms) =>
        (args contains "-v") || {
          if (line == "") (source.fullName.ToString != "scala.Predef")
          else (args exists (_root_.java.lang.String.instancehelper_contains(source.name.ToString, _)))
        }
    }
  ...
  private val crashRecovery: PartialFunction[System.Exception, Boolean] = {
  ...
  def chooseReader(settings: Settings): InteractiveReader = {
    if (settings.Xnojline.value || Properties.isEmacsShell)
      ...
      case ex @ (_: java.lang.Exception | _: java.lang.NoClassDefFoundError) =>
