...
class Power[ReplValsImpl <: ReplVals : Manifest](val intp: IMain, replVals: ReplValsImpl) {
  ...
  class PackageSlurper(pkgName: String) extends SymSlurper {
    ...
    def isIgnore(sym: Symbol) = sym.isAnonOrRefinementClass || (_root_.java.lang.String.instancehelper_contains(sym.name.ToString, "$mc"))
  ...
  class InternalInfo[T: Manifest](value: Option[T] = None) {
    ...
    def members    = tpe.members filterNot (m => _root_.java.lang.String.instancehelper_contains(m.name.ToString, "$mc"))
    ...
    def shortClass = _root_.java.lang.String.instancehelper_split(erasure.getName, "[$.]") last
  ...
  abstract class PrettifierClass[T: Prettifier]() {
    ...
    def >?(s: String): Unit                      = pp(_ filter (p => _root_.java.lang.String.instancehelper_contains(p.ToString, s)))
    def >?(r: Regex): Unit                       = pp(_ filter (p => _root_.java.lang.String.instancehelper_matches(p.ToString, fixRegex(r))))
  ...
  class RichReplString(s: String) {
    ...
    def u: URL = (
      if (_root_.java.lang.String.instancehelper_contains(s, ":")) new URL(s)
  ...
  trait Implicits2 extends Implicits1 {
    ...
    object symbolSubtypeOrdering extends Ordering[Symbol] {
      def compareNonErased(s1: Symbol, s2: Symbol) =
        if (s1 eq s2) 0
        else if (s1 isLess s2) -1
        else 1
      override def compare(s1: Any, s2: Any) = { this.compareNonErased(s1.asInstanceOf[Symbol], s2.asInstanceOf[Symbol]) }
    }
  ...
  trait ReplUtilities {
    ...
    def sanitize(s: String): String = sanitize(_root_.java.lang.String.instancehelper_getBytes(s))
