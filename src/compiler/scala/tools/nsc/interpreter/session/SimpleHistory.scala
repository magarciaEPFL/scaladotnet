...
class SimpleHistory extends JLineHistory {
  ...
  def clear() = buf.clear()
  def get(idx: Int): java.lang.CharSequence = buf(idx)
  def add(item: java.lang.CharSequence): Unit = buf += item
  ...
  def grep(s: String)      = buf.toList filter (_root_.java.lang.String.instancehelper_contains(_, s))
}
