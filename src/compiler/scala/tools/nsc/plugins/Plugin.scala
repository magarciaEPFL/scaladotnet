...
object Plugin {
   ...
   */
  private def loaderFor(jarfiles: Seq[Path]): java.lang.ClassLoader = {
   ...
   */
  def loadFrom(jarfile: Path, loader: java.lang.ClassLoader): Option[AnyClass] =
  ...
  def instantiate(clazz: AnyClass, global: Global): Plugin = {
    val constructor = clazz getConstructor scala.Array( classOf[Global] ) 
    (constructor newInstance scala.Array( global ) ).asInstanceOf[Plugin]
  }
}
