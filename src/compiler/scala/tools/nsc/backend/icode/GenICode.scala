  ...
  class ICodePhase(prev: Phase) extends StdPhase(prev) {
    ...
    private def msil_IsValuetypeInstMethod(msym: Symbol) = (
      loaders.clrTypes.methods get msym exists (mMSIL =>
        loaders.clrTypes.IsInstance(mMSIL) && mMSIL.DeclaringType.IsValueType
      )
    )
...
//            if (fun.symbol.isConstructor) Static(true) else SuperCall(mix);
        ...
        case _ =>
          abort("Unexpected tree in genLoad: " + tree + "/" + _root_.java.lang.Object.instancehelper_getClass(tree) + " at: " + tree.pos)
      }
  ...
  class Scope(val outer: Scope) {
    ...
    def varsInScope: Buffer[Local] = outer.varsInScope.MemberwiseClone() ++= locals
