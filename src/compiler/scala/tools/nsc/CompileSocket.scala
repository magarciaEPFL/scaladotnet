...
trait HasCompileSocket {
  ...
  def isErrorMessage(msg: String) = errorMarkers exists (_root_.java.lang.String.instancehelper_contains(msg, _))
...
class CompileSocket extends CompileOutputCommon {
  ...
  private def startNewServer(vmArgs: String) = {
    val cmd = serverCommand(_root_.java.lang.String.instancehelper_split(vmArgs, " ") toSeq)
      ...
      case num  =>
        ...
        Socket.localhost(port).either match {
          ...
          case Left(err)      =>
            ...
            java.lang.Thread sleep retryDelay // delay before retrying
            getsock(attempts - 1)
        }
    }
    getsock(maxAttempts)
