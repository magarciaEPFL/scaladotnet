...
class ScriptRunner extends HasCompileSocket {
        ...
        def recompile() = {
          ...
          compile match {
            case Some(compiledPath) =>
              ...
              catch { case _: java.lang.Exception => jarFile.delete() }
    ...
    scriptArgs: List[String]): Boolean =
  {
    ...
    ObjectRunner.runAndCatch(cp, scriptMain(settings), scriptArgs) match {
      case Left(ex) => java.lang.Throwable.instancehelper_printStackTrace(ex) ; false
      case _        => true
    }
  }
  ...
  def runScriptAndCatch(
    settings: GenericRunnerSettings,
    scriptFile: String,
    scriptArgs: List[String]): Either[System.Exception, Boolean] =
  {
