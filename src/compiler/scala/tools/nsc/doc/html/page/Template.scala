...
class Template(tpl: DocTemplateEntity) extends HtmlPage {
      ...
      <span class="symbol">
        {
          ...
          if (!_root_.java.lang.String.instancehelper_isEmpty(nameLink))
  ...
  def treeToHtml(tree: TreeEntity): NodeSeq = {
    ...
    for ((from, (member, to)) <- tree.refEntity.toSeq) {
      if (index < from) {
        myXml ++= codeStringToXml(_root_.java.lang.String.instancehelper_substring(str, index,from))
        index = from
      }
      if (index == from) {
        member match {
          case mbr: DocTemplateEntity =>
            ...
            myXml ++= <span class="name"><a href={link}>{_root_.java.lang.String.instancehelper_substring(str, from, to)}</a></span>
          case mbr: MemberEntity =>
            val anchor = "#" + mbr.name + defParamsString(mbr) + ":" + mbr.resultType.name
            val link = relativeLinkTo(mbr.inTemplate)
            myXml ++= <span class="name"><a href={link ++ anchor}>{_root_.java.lang.String.instancehelper_substring(str, from, to)}</a></span>
        }
        index = to
      }
    }
    ...
    if (index <= length-1)
      myXml ++= codeStringToXml(_root_.java.lang.String.instancehelper_substring(str, index, length))
