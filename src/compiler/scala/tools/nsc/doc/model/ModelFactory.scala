...
class ModelFactory(val global: Global, val settings: doc.Settings) {
  ...
  def optimize(str: String): String =
    if (str.Length < 16) _root_.java.lang.String.instancehelper_intern(str) else str
  ...
  abstract class DocTemplateImpl(sym: Symbol, inTpl: => DocTemplateImpl) extends MemberImpl(sym, inTpl) with TemplateImpl with HigherKindedImpl with DocTemplateEntity {
    ...
    def sourceUrl = {
      def fixPath(s: String) = _root_.java.lang.String.instancehelper_replaceAll(s, "\\" + java.io.File.separator, "/")
      ...
      if (!settings.docsourceurl.isDefault)
        inSource map { case (file, _) =>
          val filePath = _root_.java.lang.String.instancehelper_replaceFirst(fixPath(file.path), "^" + assumedSourceRoot, "").stripSuffix(".scala")
  ...
  abstract class NonTemplateParamMemberImpl(sym: Symbol, inTpl: => DocTemplateImpl) extends NonTemplateMemberImpl(sym, inTpl) {
    def valueParams =
      sym.paramss map { ps => (ps.zipWithIndex) map { case (p, i) =>
        if (_root_.java.lang.String.instancehelper_contains(p.nameString, "$")) makeValueParam(p, inTpl, optimize("arg" + i)) else makeValueParam(p, inTpl)
      }}
  }
  ...
  def makeTemplate(aSym: Symbol): TemplateImpl = {
    ...
    if (bSym == RootPackage)
      ...
      makeTemplate(bSym.owner) match {
        ...
        case _ => throw new java.lang.Error("'" + bSym + "' must be in a package")
      }
    else if (templateShouldDocument(bSym))
      makeTemplate(bSym.owner) match {
        case inDTpl: DocTemplateImpl => makeDocTemplate(bSym, inDTpl)
        case _ => throw new java.lang.Error("'" + bSym + "' must be in documentable template")
      }
    else
    ...
    val minimumInTpl =
      if (bSym.owner != inTpl.sym)
        makeTemplate(aSym.owner) match {
          ...
          case inNDTpl => throw new java.lang.Error("'" + bSym + "' is owned by '" + inNDTpl + "' which is not documented")
        }
      else
        inTpl
    if (templatesCache isDefinedAt (bSym))
      ...
      throw new java.lang.Error("'" + bSym + "' that isn't a class, trait or object cannot be built as a documentable template")
  }
