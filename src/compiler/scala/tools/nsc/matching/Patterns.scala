...
trait Patterns extends ast.TreeDSL {
  ...
  private def abortUnknownTree(tree: Tree) =
    abort("Unknown Tree reached pattern matcher: %s/%s".format(tree, _root_.java.lang.Object.instancehelper_getClass(tree)))
  ...
  sealed abstract class Pattern extends PatternBindingLogic {
    ...
    override def GetHashCode() = boundTree.GetHashCode
