...
class Demo(args: List[String]) extends {
  ...
  def demoSpecMethods = _root_.java.lang.Object.instancehelper_getClass(this).getMethods.toList
  private def isDemo(m: Method) = (m.getName StartsWith "opt") && !(_root_.java.lang.String.instancehelper_contains(m.getName, "$")) && (m.getParameterTypes.isEmpty)

  def demoString(ms: List[Method]) = {
    val longest   = ms map (m => m.getName.Length) max
    val formatStr = "    %-" + longest + "s: %s"
    val xs        = ms map (m => formatStr.format(m.getName, m.invoke(this, scala.Array.empty[Object])))
