  ...
  def unpickle(bytes: Array[Byte], offset: Int, classRoot: Symbol, moduleRoot: Symbol, filename: String) {
    try {
      ...
      case ex: System.Exception =>
        ...
        throw new java.lang.RuntimeException("error reading Scala signature of "+filename+": "+ex.Message)
    }
  }
  ...
  class Scan(_bytes: Array[Byte], offset: Int, classRoot: Symbol, moduleRoot: Symbol, filename: String) extends PickleBuffer(_bytes, offset, -1) {
        ...
        case POLYtpe =>
          ...
          if(typeParams nonEmpty) {
            ...
            def transitionNMT(restpe: Type) = {
              val resTpeCls = _root_.java.lang.Object.instancehelper_getClass(restpe).ToString // what's uglier than isInstanceOf? right! -- isInstanceOf does not work since the concrete types are defined in the compiler (not in scope here)
              if(forceProperType /*&& pickleformat < 2.9 */ && !(_root_.java.lang.String.instancehelper_endsWith(resTpeCls, "MethodType"))) { assert(!_root_.java.lang.String.instancehelper_contains(resTpeCls, "ClassInfoType"))
    ...
    protected def readConstant(): Constant = {
      ...
      (tag: @switch) match {
        ...
        case LITERALfloat   => Constant(java.lang.Float.intBitsToFloat(readLong(len).toInt))
        case LITERALdouble  => Constant(java.lang.Double.longBitsToDouble(readLong(len)))
    ...
    def readModifiers(): Modifiers = {
      ...
      val pflags = (pflagsHi.toLong << ((32) & 0x3f)) + pflagsLo
    ...
    protected def errorBadSignature(msg: String) =
      throw new java.lang.RuntimeException("malformed Scala signature of " + classRoot.name + " at " + readIndex + "; " + msg)
