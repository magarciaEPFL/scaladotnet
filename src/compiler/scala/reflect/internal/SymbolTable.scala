                              ...
                              with Required
{
  ...
  final def runId(period: Period): RunId = period >> ((8) & 0x1f)
  ...
  object perRunCaches {
    ...
    private def dumpCaches() {
      ...
      caches.zipWithIndex foreach { case (ref, index) =>
        val cache = ref.get().asInstanceOf[Clearable] /* MANUALLY */
        println("(" + index + ")" + (
          if (cache == null) " has been collected."
          else " has entries:\n" + stringOf(cache) /* MANUALLY */
        ))
      }
    }
