...
trait StdNames extends NameManglers { self: SymbolTable =>
   ...
   */
  object binarynme {
    def toBinary(name: Name) = name mapName (_root_.java.lang.String.instancehelper_replace(_, '.', '/'))
  ...
  object nme extends TermNames /*with LibraryTermNames*/ with TermNameMangling {
    ...
    def segments(name: String, assumeTerm: Boolean): List[Name] = {
      ...
      name.indexWhere(ch => ch == '.' || ch == '#') match {
        ...
        case idx    =>
          val (simple, div, rest) = (name take idx, name Chars idx, newTermName(name) drop (idx + 1))
