...
trait Chars {
  ...
  def char2uescape(c: Char): String = {
    ...
    char2uescapeArray(2) = hexChar((c >> ((12) & 0x1f))     )
    char2uescapeArray(3) = hexChar((c >>  ((8) & 0x1f)) % 16)
    char2uescapeArray(4) = hexChar((c >>  ((4) & 0x1f)) % 16)
    char2uescapeArray(5) = hexChar((c      ) % 16)

    java.lang.String.newhelper(char2uescapeArray) 
  }
  ...
  def isIdentifierStart(c: Char): Boolean =
    (c == '_') || (c == '$') || java.lang.Character.isUnicodeIdentifierStart(c)
  ...
  def isSpecial(c: Char) = {
    val chtp = java.lang.Character.getType(c)
  ...
  def isScalaLetter(ch: Char) = letterGroups(java.lang.Character.getType(ch).toByte) || otherLetters(ch)
