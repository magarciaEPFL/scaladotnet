...
trait SynchronizedTypes extends internal.Types { self: SymbolTable =>
  ...
  private val uniqueLock = new AnyRef
  override def unique[T <: Type](tp: T): T = (uniqueLock.synchronized { super.unique(tp) }).asInstanceOf[T]
  ...
  class SynchronizedUndoLog extends UndoLog {
    ...
    override def undo[T](block: => T): T =
      (synchronized { super.undo(block) }).asInstanceOf[T]
  ...
  private val subsametypeLock = new AnyRef  
  ...
  private val lubglbLock = new AnyRef
  ...
  private val indentLock = new AnyRef
  ...
  private val toStringLock = new AnyRef
