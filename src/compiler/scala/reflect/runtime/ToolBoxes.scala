...
trait ToolBoxes extends { self: Universe =>
  ...
  class ToolBox(val reporter: Reporter = new StoreReporter, val options: String = "") {
    ...
    extends ReflectGlobal(settings, reporter) {
      ...
      def runExpr(expr: Tree): Any = {
        ...
        if (reporter.hasErrors) {
          throw new java.lang.Error("reflective compilation has failed")
        }
        
        if (settings.debug.value) println("generated: "+className)
        val jclazz = java.lang.Class.forName(moduleFileName(className), true, classLoader)
        ...
        val result = jmeth.invoke(singleton, scala.Array( fvs map (sym => sym.asInstanceOf[FreeVar].value.asInstanceOf[AnyRef]): _* ) )
        if (etpe.typeSymbol != FunctionClass(0)) result
        else {
          val applyMeth = _root_.java.lang.Object.instancehelper_getClass(result).getMethod("apply", scala.Array.empty[java.lang.Class]) /* MANUALLY */
          applyMeth.invoke(result, scala.Array.empty[Object])
        }
      }
    ...
    lazy val arguments = _root_.java.lang.String.instancehelper_split(options, " ")
